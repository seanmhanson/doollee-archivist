...
async function testRateLimit() {
  ...
  for (const delay of delays) {
    console.log(`\n📊 Testing ${delay}ms delay between requests...`);
    ...
    for (let i = 0; i < testUrls.length && !shouldBreak; i++) {
      ...
      try {
        ...
        if (status === 429) {
          console.warn("⛔ Rate limit detected (429)");
          shouldBreak = true;
        } else if (status === 503 || status === 502) {
          console.warn("⚠️  Server error - may be overloaded");
          shouldBreak = true;
        } else if (responseTime > 15000) {
          console.warn("⚠️  Very slow response - server may be stressed");
    ...
    if (delayMetrics.length > 0) {
      ...
      if (average > 8000) {
        console.warn("⚠️  High response times - consider longer delays");
      }
    }

    if (shouldBreak) {
      console.log("🛑 Stopping tests due to errors or rate limiting\n");
      break;
    }

    // Brief pause between delay tests
    console.log("⏸️  Pausing 10s before next delay test...");
  ...
  if (successfulMetrics.length === 0) {
    console.log("❌ No successful requests - site may be down or blocking");
    return;
  }
